...
(function($) {
    $.fn.sudyNav = function() {
        ...
        return this.each(function() {
                ...
                var l = c.find(".nav-bar");
                var u = c.find("ul");
                var su = u.find("ul");
                var cl = c.children('.nav-bar');
                var cu = c.children('.nav-bar').children('ul');
                var isIE6 = !-[1, ] && !window.XMLHttpRequest;
                if (o.vertical.active == '1') {
                    c.css({height: 'auto'}).children('.nav-bar').css({display: "block", width: (100 / o.vertical.col) + '%', float: 'none'}).children('a').css({display: "block", width: "auto"});
                    if (o.vertical.col !== '1') {
                        c.children(".nav-bar").css({display: "inline-block", float: 'left'});
                    }
                    if (o.drop_v == 'up') {
                        u.css({bottom: '0px'});
                    } else {
                ...
                $.each(u, function() {
                    var sl = $(this).children(".nav-bar"), suW = 0, suH = 0, sW = 0, sH = 0;
                    $.each(sl, function() {
